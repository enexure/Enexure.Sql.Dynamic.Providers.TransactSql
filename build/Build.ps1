...
task Compile -depends Version, Clean { 
	...
	$solutionPath = "$solutionDir\Enexure.Sql.Dynamic.Providers.TransactSql.sln"
	Invoke-MsBuild $solutionPath -MSBuildProperties @{ Configuration = "Release" }
}

task Version -depends Clean {

	$versionSourceFile = "$solutionDir\src\Enexure.Sql.Dynamic.Providers.TransactSql\Version.json"
	...
	$projectDir = "$solutionDir\src\Enexure.Sql.Dynamic.Providers.TransactSql\Properties"
